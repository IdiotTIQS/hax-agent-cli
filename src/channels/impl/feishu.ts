...
class FeishuAdapter extends ChannelAdapter {
  ...
  constructor(cfg: FeishuConfig = {}) {
    super({ name: "feishu", ...cfg });
    this._appId = cfg.appId || "";
    this._appSecret = cfg.appSecret || "";
    this._domain = cfg.domain || "https://open.feishu.cn";
  }
  ...
  async send(target: string | null, message: SendMessage): Promise<SendResult> {
    ...
    try {
      const tr = await fetch(
        this._domain + "/open-apis/auth/v3/tenant_access_token/internal",
        { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ app_id: this._appId, app_secret: this._appSecret }) }
      );
      const td = await tr.json() as { tenant_access_token?: string };
      ...
      const r = await fetch(
        this._domain + "/open-apis/im/v1/messages?receive_id_type=chat_id",
        {
          method: "POST",
          headers: { "Content-Type": "application/json", Authorization: "Bearer " + td.tenant_access_token },
