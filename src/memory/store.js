...
class MemoryEntry {
  ...
  _computeSignature(title, content) {
...
class MemoryStore {
  ...
  async merge(id, newContent, opts = {}) {
    await this.init();
    const e = this._entries.get(id);
    if (!e) return null;
  ...
  static buildExtractionPrompt(recentMessages, existingTitles = []) {
  ...
  _path(entry) { return path.join(this._dir, `${entry.id}${EXT}`); }
  _indexPath() { return path.join(this._dir, INDEX_FILE); }
